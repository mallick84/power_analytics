          ...
          fluidRow(
            column(
              ...
              checkboxInput(
                ...
                label = "Help us make Codingobjects better",
 ...
 # Defining & initializing the reactiveValues object
  ...
  output$total_cost <- renderText({
    if(input$submit_button == 0) {
      ...
      use your own API key (Settings), or help cover the fee via PayPal(info@codingobjects.in)."
      )
    } else {
    #req(openAI_response()$cmd)
      paste0(
...
[openai](https://cran.rstudio.com/web/packages/openai/index.html)
    ...
    tagList(
      ...
      fluidRow(
        column(
          width = 3,
          downloadButton(
            outputId = "eda_report_Codingobjects",
            label = "EDA Report"
          )
        ),
        column(
  ...
  # Markdown report
  output$eda_report_Codingobjects <- downloadHandler(
    # For PDF output, change this to "report.pdf"
    filename = "Codingobjects_EDA.html",
    content = function(file) {
      withProgress(message = "Generating Report (5 minutes)", {
        incProgress(0.2)
        ...
        tempReport <- file.path(tempdir(), "Codingobjects_EDA.Rmd")
        # tempReport
  ...
  output$Rmd_source <- downloadHandler(
    # For PDF output, change this to "report.pdf"
    filename = "Codingobjects.Rmd",
    content = function(file) {
      Rmd_script <- paste0(
        "---\n",
        "title: \"Codingobjects report\"\n",
        "author: \"Codingobjects, Powered by ChatGPT\"\n",
        "date: \"",
        date(), "\"\n",
  ...
  output$report <- downloadHandler(
    # For PDF output, change this to "report.pdf"
    filename = "Codingobjects_report.html",
    content = function(file) {
      withProgress(message = "Generating Report ...", {
        ...
        Rmd_script <- paste0(
          "---\n",
          "title: \"Codingobjects.ai report\"\n",
          "author: \"Codingobjects v.",
          release,
          ", Powered by ChatGPT\"\n",
          "date: \"",
          date(), "\"\n",
...
#______________________________________________________________________________
  ...
  output$Codingobjects_version <- renderUI({
    h4(paste("Codingobjects Version", release))
  })

  output$Codingobjects_version_main <- renderUI({
    tagList(
      h3(paste("Codingobjects.ai ", release))
    )
  })
  ...
  show_pop_up <- function() {
    showModal(
      modalDialog(
        ...
        it contains numbers. For columns that are numbers, but with few unique values, Codingobjects
...
show_pop_up_2 <- function() {
    showModal(
      modalDialog(
        ...
        automatically converted. For columns that are numbers, but with few unique values, Codingobjects
        automatically convert them to factors. See Settings.",
        style = "color: blue"),
        br(),
        footer = tagList(
